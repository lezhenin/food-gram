...
from filter import OrderOwnerFilter, UserStateFilter, ChatStateFilter, ChatTypeFilter
...
@dp.message_handler(commands=['show'], chat_type='private', is_order_owner=True)
async def show_order(message: types.Message):
    ...
    message_text = UserState+'Блюда в заказе:\n' + '\n'.join(dishes) if (len(dishes) > 0) else 'Ваш заказ пуст'
    await bot.send_message(message.from_user.id, message_text)
